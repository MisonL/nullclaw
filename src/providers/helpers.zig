...
/// Resolve max_tokens from config-like structs that may expose either
/// `max_tokens: u32` or `max_tokens: ?u32`.
fn resolveMaxTokensFromCfg(cfg: anytype) u32 {
    const T = @TypeOf(cfg.max_tokens);
    return switch (@typeInfo(T)) {
        .optional => blk: {
            if (cfg.max_tokens) |mt| {
                break :blk @intCast(@min(mt, std.math.maxInt(u32)));
            }
            break :blk 4096;
        },
        else => @intCast(@min(cfg.max_tokens, std.math.maxInt(u32))),
    };
}
...
pub fn complete(allocator: std.mem.Allocator, cfg: anytype, prompt: []const u8) ![]const u8 {
    ...
    const body_str = try buildRequestBody(allocator, model, prompt, cfg.temperature, resolveMaxTokensFromCfg(cfg));
...
pub fn completeWithSystem(allocator: std.mem.Allocator, cfg: anytype, system_prompt: []const u8, prompt: []const u8) ![]const u8 {
    ...
    const url = providerUrl(cfg.default_provider);
    const model = cfg.default_model orelse "anthropic/claude-sonnet-4-5-20250929";
    const max_tok: u32 = resolveMaxTokensFromCfg(cfg);
