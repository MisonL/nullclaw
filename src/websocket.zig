...
pub const WsClient = struct {
    ...
    pub fn connect(
        ...
        while (resp_len < resp_buf.len) {
            const n = tls_state.tls_client.reader.readSliceShort(resp_buf[resp_len..]) catch
    ...
    fn readExact(self: *WsClient, buf: []u8) !void {
        var total: usize = 0;
        while (total < buf.len) {
            const n = self.tls.tls_client.reader.readSliceShort(buf[total..]) catch |err| return err;
